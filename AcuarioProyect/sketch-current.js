...
function setup() {
    ...
    console.log('Setup completado - params:', params);
    console.log('Sliders deberían estar listos. Prueba moverlos.');
    
    // Verificar que los sliders existen
    console.log('Vision slider exists:', document.getElementById('vision') !== null);
    console.log('Cohesion slider exists:', document.getElementById('cohesion') !== null);
}

// ============================================
// FUNCIÓN PARA ACTUALIZAR CANTIDAD DE PECES
// ============================================
...
function mousePressed() {
    // Definir áreas excluidas donde NO se puede generar comida:
    // - Botones circulares (abajo derecha): aprox últimos 80px desde la derecha, últimos 130px desde abajo
    // - Toggles (arriba derecha): aprox últimos 170px desde la derecha, primeros 140px desde arriba
    // - Panel de datos oceanográficos (abajo izquierda): primeros 170px desde izquierda, últimos 100px desde abajo
    
    const excludeBottomRight = mouseX > width - 80 && mouseY > height - 130; // Botones circulares
    const excludeTopRight = mouseX > width - 170 && mouseY < 140; // Toggles
    const excludeBottomLeft = mouseX < 170 && mouseY > height - 100; // Panel oceánico
    
    // Solo generar pellets si:
    // 1. El mouse está dentro del canvas
    // 2. NO está en ninguna área excluida
    if (mouseX >= 0 && mouseX <= width && mouseY >= 0 && mouseY <= height) {
        if (!excludeBottomRight && !excludeTopRight && !excludeBottomLeft) {
    ...
    // Prevenir el comportamiento por defecto para evitar conflictos con elementos HTML
    return false;
}
